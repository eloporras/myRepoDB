...
public interface RequestedItemSQL {
    	...
    	public static final String GET_ALL_FOR_REQUESTED_SITE = "SELECT * FROM RequestedItem fi JOIN Item i ON fi.ItemId = i.ItemId JOIN Site s ON i.SiteId = s.Id LEFT JOIN FoodItem ON FoodItem.ItemId = i.ItemId LEFT JOIN SupplyItem ON SupplyItem.ItemId = i.ItemId WHERE i.SiteId = ? AND fi.Status = ?";

    	public static final String GET_BY_REQUESTID = "SELECT * FROM RequestedItem fi JOIN Item i ON fi.ItemId = i.ItemId JOIN Site s ON fi.RequesteeSiteId = s.Id LEFT JOIN FoodItem ON FoodItem.ItemId = i.ItemId LEFT JOIN SupplyItem ON SupplyItem.ItemId = i.ItemId WHERE fi.RequestId = ?";
