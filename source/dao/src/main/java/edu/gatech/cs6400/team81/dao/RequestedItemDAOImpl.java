...
public class RequestedItemDAOImpl extends BaseDAO<RequestedItem> implements RequestedItemDAO, RequestedItemSQL{
    ...
    protected RequestedItem mapResult(ResultSet rs) throws SQLException {
        ...
        requestedItem.setRequestId(rs.getInt(REQUESTID));
        requestedItem.setItemId(rs.getInt(ITEMID));
        requestedItem.setCategory(category);
        requestedItem.setExpireDate(rs.getDate(ItemDAO.EXPIREDATE));
        requestedItem.setName(rs.getString(ItemDAO.NAME));
        requestedItem.setNumberUnits(rs.getInt(ItemDAO.NUMBERUNITS));
        ...
        requestedItem.setNumberFilled(rs.getInt(RequestedItemDAO.NUMFILLED));
        requestedItem.setNumberRequested(rs.getInt(RequestedItemDAO.NUMREQUESTED));
    ...
    @Override
    public List<RequestedItem> getAllForOwnSiteAndStatus(int siteId, RequestedItemStatus status) throws SQLException {
        return getMultiple(GET_ALL_FOR_REQUESTED_SITE, new Object[]{siteId, status.getValue()});
    }
    
    @Override
    public RequestedItem getByRequestId(int requestId) throws SQLException {
        return getUnique(GET_BY_REQUESTID, new Object[]{requestId});
    }
