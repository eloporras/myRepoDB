...
public class MakeFoodBankRequestServlet extends BaseServlet implements Servlet{
    ...
    protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
    	if("Submit Request".equalsIgnoreCase(request.getParameter("action"))){			
    		...
    		User user = getLoggedOnUser(request);
    		try {
    			List<ClientService> clientServices = clientServiceDAO.getBySiteId(site.getId());
    			if(CollectionUtils.isEmpty(clientServices)){
    				response.getWriter().write(("Must have atleast one Client Service [Food Pantry, Shelter or Soup Kitchen] to Request Food Bank Items"));			
    			} else {
